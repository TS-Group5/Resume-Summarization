...
class ATSParser(BaseParser):
        ...
        for pattern, date_format in date_patterns:
            ...
            if match:
                try:
                    ...
                    logger.debug(
                        f"Successfully parsed date: {date_str} -> {parsed_date}"
                    )
                    return parsed_date
    ...
    def calculate_years_experience(self, dates):
        ...
        for start_date, end_date in dates:
            if not start_date:
                logger.debug(
                    f"Skipping date range due to missing start date: {start_date} - {end_date}"
                )
                continue
    ...
    def _extract_name(self, text: str) -> str:
        ...
        for line in lines[:3]:  # Check first 3 lines
            ...
            if len(words) >= 2 and all(w[0].isupper() for w in words if w):
                ...
                if not any(
                    x in line.lower() for x in ["resume", "cv", "curriculum vitae"]
                ):
                    return line.strip()
        return ""
    ...
    def _extract_role(self, text: str) -> str:
        ...
        summary_patterns = [
            ...
            r"(?i)(?:^|\n)(?:I am|Currently|Now|Presently)?\s*(?:a|an)?\s*([A-Z][a-zA-Z\s]+(?:"
            + "|".join(job_titles)
            + r")(?:\s+at|\s+with|\s+for|\s+in|\s*\n|\s*$|\.))",
            r"(?i)current\s+(?:role|position|title):\s*([^\n]+)",
            r"(?i)(?:^|\n)experienced\s+([A-Z][a-zA-Z\s]+(?:"
            + "|".join(job_titles)
            + r")(?:\s+with|\s+having|\.))",
            r"(?i)(?:^|\n)([A-Z][a-zA-Z\s]+(?:"
            + "|".join(job_titles)
            + r"))\s+with\s+\d+\+?\s*years?",
        ...
        if experience_section:
            ...
            for line in lines[:3]:  # Check first few lines
                ...
                for title in job_titles:
                    match = re.search(
                        rf"(?i)([A-Z][a-zA-Z\s]*{title}[a-zA-Z\s]*)", line
                    )
                    if match:
                        role = match.group(1).strip()
                        if 2 <= len(role.split()) <= 6:
        ...
        achievements_section = re.search(
            r"(?i)(?:achievements|responsibilities|key\s+accomplishments)[:\n]+(.*?)(?:\n\n|\Z)",
            text,
            re.DOTALL,
        )
    ...
    def _extract_skills(self, text: str) -> List[str]:
        ...
        skills_section = re.search(
            r"(?i)(?:skills|expertise|proficiencies)[:\n]+(.*?)(?:\n\n|\Z)",
            text,
            re.DOTALL,
        )
        if skills_section:
            ...
            for skill in skill_list:
                ...
                if skill:
                    ...
                    if (
                        skill in self.TECHNICAL_SKILLS
                        or skill in self.BUSINESS_SKILLS
                        or skill in self.CREATIVE_SKILLS
                        or skill in self.HEALTHCARE_SKILLS
                        or skill in self.HR_SKILLS
                        or skill in self.SOFT_SKILLS
                        or skill in self.EDUCATION_SKILLS
                    ):
                        skills.add(skill)
        ...
        for pattern in skill_patterns:
            ...
            for match in matches:
                ...
                if (
                    not any(irr in skill for irr in self.IRRELEVANT_TERMS)
        ...
        for pattern in company_patterns:
            ...
            for match in matches:
                ...
                if company and company not in companies:
                    ...
                    if not any(
                        x in company.lower()
                        for x in ["resume", "cv", "summary", "profile", "experience"]
                    ):
    ...
    def _extract_achievements(self, text: str) -> List[Dict[str, Any]]:
        ...
        for sentence in sentences:
            ...
            if achievement_data["score"] > 0:
                ...
                cleaned_text = re.sub(
                    r"\s*\|\s*", " at ", cleaned_text
                )  # Replace | with 'at'
                cleaned_text = re.sub(
                    r"\s*[-–]\s*", " to ", cleaned_text
                )  # Replace - with 'to'
                cleaned_text = re.sub(
    ...
    def _format_achievement(self, achievement: Dict[str, Any]) -> str:
        ...
        if achievement["categories"]:
            primary_category = max(
                achievement["categories"].items(), key=lambda x: x[1]
            )[0]
        ...
        for pattern in phone_patterns:
            ...
            if phone_match:
                ...
                if len(phone) >= 10:  # Ensure it's a valid length
                    contact_info["phone"] = (
                        f"({phone[-10:-7]}) {phone[-7:-4]}-{phone[-4:]}"
                    )
                break
    ...
    def _is_contact_info(self, text: str) -> bool:
        ...
        return any(
            re.search(pattern, text, re.IGNORECASE) for pattern in contact_patterns
        )
    ...
    def _is_date_or_role(self, text: str) -> bool:
        ...
        return any(
            re.search(pattern, text, re.IGNORECASE) for pattern in date_role_patterns
        )
    ...
    def _process_content(self, text_content: List[str]) -> None:
        ...
        try:
            ...
            self.resume_data["years_experience"] = self._extract_years_experience(
                full_text
            )
            ...
            for line in text_content:
                ...
                if education_section:
                    ...
                    degree_match = re.search(
                        r"(?:Bachelor's|Master's|PhD|B\.[A-Z]|M\.[A-Z]|Ph\.D)\s+(?:of|in|degree in)?\s+([^\n]+)",
                        line,
                    )
                    if degree_match:
